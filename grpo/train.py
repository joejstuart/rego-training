...
Architecture (adapted from the Unsloth Qwen3-14B GRPO notebook):
...
Usage:
    ...
    python -m grpo.train --sft-model ./sft/output/rego-expert-14b
...
DEFAULT_SFT_MODEL = str(SFT_ROOT / "output" / "rego-expert-14b")
DEFAULT_OUTPUT_DIR = str(GRPO_DIR / "output" / "rego-expert-grpo-14b")
...
def main() -> None:
    ...
    # tensor size mismatches in Unsloth's GRPOTrainer
    tokenized_filtered = dataset.map(
        lambda x: {
            "tokens": tokenizer.apply_chat_template(
                x["prompt"], add_generation_prompt=True, tokenize=True,
            )
    ...
    # potential padding/alignment issues in Unsloth's GRPOTrainer.
    ...
    if max_completion_length < 100:
        print(f"WARNING: max_completion_length ({max_completion_length}) is very small.")
